...
const importTemplate = (comp_name, style, index) => ({
    ...
    inputTemplateName: `template_${index + 1}`,
    staticTemplateName: `template_${index + 1}`
});
...
for (const style in raw_metadata) {
    ...
    for (const compType in raw_metadata[style]) {
        ...
        Object.values(raw_metadata[style][compType]).map((component, index) => {
            ...
            const inputTemplateName = component.inputTemplateName;
            const staticTemplateName = component.staticTemplateName;
